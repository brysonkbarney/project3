...
app.get("/add", (req, res) => {
  res.render("add"); // Render the add.ejs file
});
...
app.post("/storeData", upload.single("activityImage"), (req, res) => {
  ...
  knex("activity")
    .insert({
      ...
      equipment_description:
        equipment === "True" ? equipment_description : null,
      price: price,
      location: location,
      type: type,
    ...
    .then(() => {
      res.send(
        `<script>alert("Activity added successfully."); window.location.href = "/"; </script>`
      );
    })
    .catch((err) => {
      console.error(err);
      res
        .status(500)
        .send(
          `<script>alert("Error adding activity to the database."); window.location.href = "/add"; </script>`
        );
    });
});
...
app.get("/filter", (req, res) => {
  ...
  if (
    req.query.location &&
    req.query.location !== "" &&
    req.query.location !== "Doesn't Matter"
  ) {
  ...
  if (
    ...
    req.query.time_of_day !== "" &&
    req.query.time_of_day !== "Doesn't Matter"
  ) {
...
// Handling GET request for the account page
app.get("/account", (req, res) => {
  // Checking if the user is logged in
  const isLoggedIn = req.session.loggedIn || false;
  // Rendering the account page and passing the logged-in status and userinfo to the EJS template
  res.render("account", {
...
app.get("/getRandomRecord", async (req, res) => {
  try {
    ...
    // Step 2: Generate a random number within the bounds (considering records start at 1005)
    const randomIndex = Math.floor(Math.random() * totalCount) + 1022;
...
app.post("/updateRecord/:id", upload.single("activityImage"), (req, res) => {
  ...
  knex("activity")
    ...
    .then(() => {
      res.send(
        `<script>alert("Activity updated successfully."); window.location.href = "/activity/${activityId}"; </script>`
      );
    })
    .catch((err) => {
      console.error(err);
      res
        .status(500)
        .send(
          `<script>alert("Error updating activity."); window.location.href = "/editRecord/${activityId}"; </script>`
        );
    });
});
...
app.get("/deleteRecord/:id", (req, res) => {
  ...
  knex("activity")
    ...
    .then(() => {
      res.send(
        `<script>alert("Activity deleted successfully."); window.location.href = "/"; </script>`
      );
    })
    .catch((err) => {
      console.error(err);
